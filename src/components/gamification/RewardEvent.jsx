...
export default function RewardEvent({ event, dark, languageId, soundEnabled, onDone }) {
  const reduceMotion = useReducedMotion();
  useEffect(() => {
    if (!event) return undefined;
    if (event.kind === "milestone") playUiSound("unit", soundEnabled);
    else if (event.kind === "xp" || event.kind === "heart-gain") playUiSound("reward", soundEnabled);
    const timer = window.setTimeout(onDone, event.kind === "milestone" ? 3200 : 1900);
    return () => window.clearTimeout(timer);
  }, [event, onDone, soundEnabled]);
