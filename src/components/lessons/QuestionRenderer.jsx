...
function SentenceBuilder({ question, value, onChange, dark, checked }) {
  ...
  return <div><div className={`min-h-20 rounded-2xl border-2 border-dashed p-3 ${dark ? "border-white/15" : "border-black/15"}`}>{value.map((word, i) => <button disabled={checked} key={`${word}-${i}`} onClick={() => onChange(value.filter((_, n) => n !== i))} className="m-1 rounded-xl bg-[#F28C28] px-4 py-3 font-black text-white">{word}</button>)}</div><div className="mt-4 flex flex-wrap gap-2">{tiles.map((word, i) => <button disabled={checked || value.filter(v => v === word).length >= tiles.filter((v, n) => v === word && n <= i).length} key={`${word}-${i}`} onClick={() => onChange([...value, word])} className={`rounded-xl px-4 py-3 font-black disabled:opacity-25 ${dark ? "bg-white/10" : "bg-black/8"}`}>{word}</button>)}</div></div>;
}
...
function Matching({ question, value, onChange, dark, checked }) {
  ...
  return <div className="grid grid-cols-2 gap-3"><div className="space-y-2">{pairs.map(p => <button key={p.native} disabled={checked || usedNative.has(p.native)} onClick={() => setNative(p.native)} className={`min-h-14 w-full rounded-xl p-3 font-black ${native === p.native ? "bg-[#F28C28] text-white" : dark ? "bg-white/8" : "bg-black/5"} disabled:opacity-30`}>{p.native}</button>)}</div><div className="space-y-2">{pairs.map(p => <button key={p.english} disabled={checked || usedEnglish.has(p.english)} onClick={() => add(p.english)} className={`min-h-14 w-full rounded-xl p-3 font-black ${dark ? "bg-white/8" : "bg-black/5"} disabled:opacity-30`}>{p.english}</button>)}</div></div>;
}
