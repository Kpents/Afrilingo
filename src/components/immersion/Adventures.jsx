...
function AdventureScene({ adventure, languageId, dark, hearts, completed, soundEnabled, onLoseHeart, onExit, onReward }) {
  ...
  const [metPerson, setMetPerson] = useState(null);
  ...
  return <div className="mx-auto max-w-2xl"><div className="mb-4 flex items-center gap-3"><button onClick={onExit} aria-label="Exit adventure" className={`grid size-11 place-items-center rounded-xl ${dark?"bg-white/6":"bg-black/5"}`}><ArrowLeft size={20}/></button><div><div className="text-xs font-black uppercase tracking-wider text-[#F28C28]">{adventure.level} adventure</div><div className="font-black">{adventure.title}</div></div></div>
    <div className="relative mx-auto aspect-[3/5] w-full max-w-[28rem] overflow-hidden rounded-[2rem] border border-black/10 bg-[#F6C445]/20 shadow-2xl"><img src={`${import.meta.env.BASE_URL}${adventure.image}`} alt={`Illustrated ${adventure.title} setting`} className="absolute inset-0 h-full w-full object-cover object-center"/>
      ...
      <AnimatePresence>{step === "talk" && <motion.div ref={dialogueRef} initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} className={`absolute inset-x-3 bottom-3 z-20 max-h-[58%] overflow-y-auto rounded-[1.6rem] border p-4 shadow-2xl ${dark ? "border-white/10 bg-[#1A201E]/95" : "border-black/10 bg-white/95"}`}>
        ...
        {hearts <= 0 && !correct && <div className="mt-3 rounded-xl bg-[#C95D3A]/15 p-3 text-sm font-bold">Out of hearts. Return to Immersion and recover a heart before trying again.</div>}
        {choice && <div className={`mt-3 flex gap-2 rounded-xl p-3 text-sm font-semibold ${correct ? "bg-[#24745B]/15" : "bg-[#C95D3A]/15"}`}>{correct ? <CheckCircle2 className="shrink-0 text-[#53B98A]"/> : <XCircle className="shrink-0 text-[#C95D3A]"/>}<span>{correct ? adventure.turn.feedback : "That response does not fit yet. Try another choice."}</span></div>}
        {choice && <CompanionReaction character={adventure.companion} correct={correct} question={adventure.turn.english} hearts={hearts} choice={choice}/>}
        {correct && <button onClick={finish} onPointerDown={hapticPress} className="afri-press mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#F28C28] font-black text-white"><Sparkles size={18}/> Complete · +{completed ? 0 : adventure.xp} XP</button>}
